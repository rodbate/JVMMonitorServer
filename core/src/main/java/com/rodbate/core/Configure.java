...
import com.rodbate.core.util.FeatureCodec;
...
public class Configure {
    ...
    public String toString() {
        ...
        for (Field field : GaReflectUtils.getFields(Configure.class)) {
            ...
            // 非静态的才需要纳入非序列化过程
            try {
                map.put(field.getName(), GaStringUtils.newString(GaReflectUtils.getValue(this, field)));
            } catch (Throwable t) {
                //
            }
    ...
    public static Configure toConfigure(String toString) {
        ...
        for (Map.Entry<String, String> entry : map.entrySet()) {
            try {
                final Field field = GaReflectUtils.getField(Configure.class, entry.getKey());
                if (null != field
                        && !isStatic(field.getModifiers())) {
                    GaReflectUtils.setValue(field, GaReflectUtils.valueOf(field.getType(), entry.getValue()), configure);
                }
            } catch (Throwable t) {
                //
            }
